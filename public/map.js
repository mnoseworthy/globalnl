...
function renderWithoutUser() {
  if (navigator.userAgent.indexOf('gonative') > -1) { // show the map for not signed in mobile app users
    if (!infowindow) {
      infowindow = new google.maps.InfoWindow();
      // Try HTML5 geolocation.
      if (navigator.geolocation) {
    ...
    $("#map").show();
    parseCords();
  } else { // not mobile app, so hide map for not signed in users
    $("#map").hide();
  }
}
...
function parseCords() {
  ...
  } else if (_map && (navigator.userAgent.indexOf('gonative') > -1)) { // to load members on map with public privacy for not signed in mobile app users
    var bounds = _map.getBounds();
    var ne = bounds.getNorthEast(); // LatLng of the north-east corner
    var sw = bounds.getSouthWest(); // LatLng of the south-west corder
    var neCorner = { lat: ne.lat(), lng: ne.lng() };
    var swCorner = { lat: sw.lat(), lng: sw.lng() };
    firebase.auth().onAuthStateChanged(function(user) { // check if user has signed in
      if (user)          
        loadMembersOnMap(neCorner, swCorner); // user is signed in, so load all members on map as usual
      else
        loadPublicMembersOnMap(neCorner, swCorner); // user is not signed in, so load public members on map only
    });
  }
}
...
// Mobile App Exclusive: Queries database for all members, whose privacy is set public, currently located within the coordinates
// and creates a pin on the map for each member
function loadPublicMembersOnMap(ne, sw) {
  // Oddly enough, you can't filter over different fields... so we take whatever is
  // between the latitude lines and filter out the longitude locally
  fbi
